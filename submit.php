...
$sql="select * from redo where (name='$name' AND place='$place');";
      ...
      if (mysqli_num_rows($res) > 0) {
        ...
        if((($name==$row['name']) && ($contact==$row['contact'])) || (($name==$row['name']) && ($secondarycontact==$row['secondarycontact'])))
        {
            	echo "<h2>"."A patient by the <strong> Same name having Same Contact Number</strong>has already registered"."</h2>";
        } }
...
//ends here
if (strlen($contact)==10 && strlen($secondarycontact)==10) {
  // code...



  $add = "INSERT INTO redo (name,place, age, code,gender,contact,secondarycontact) VALUES ('$name','$place' , '$age', '$code','$gender','$contact','$secondarycontact');";
  ...
  if ($result) { ?>
    ...
    <br> <h5> <button type="button" name="button"><a href="login.php"> Back to Registration  </a></button> </h5>
<!--    <a class="float-right" href="logout.php">Logout</a> -->
...
<?php
  ...
  echo "<h4>"."Check contact info again"."</h4>";
}
}
}
 ?>
